...
function applyFaceSource(src, instructions, resetCallback, startTimerCallback, readyText) {
    face.image.onload = () => {
        face.ready = true;
        face.loaded = true;
        updateFaceScale();
...
function loadFaceFromFile(file, fileName, instructions, resetCallback, startTimerCallback) {
    if (!file) return;
...
function loadDefaultFace(instructions, resetCallback, startTimerCallback) {
    ...
    const src = `data:image/svg+xml;utf8,${encodeURIComponent(svgFace.trim())}`;
    applyFaceSource(
        src,
        instructions,
        resetCallback,
